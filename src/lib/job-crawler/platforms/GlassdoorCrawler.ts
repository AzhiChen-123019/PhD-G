...
class GlassdoorCrawler {
  ...
  async crawl(keywords: string, location: string, limit: number = 10): Promise<Job[]> {
    try {
      const jobs: Job[] = [];
      const url = `${this.config.baseUrl}/Job/jobs.htm?sc.keyword=${encodeURIComponent(keywords)}&sc.location=${encodeURIComponent(location)}`;
      ...
      $('.jobListing').each((index, element) => {
        ...
        const linkElement = $(element).find('a.jobLink');
      ...
      for (const job of jobs) {
        if (job.url) {
          try {
            ...
            const descriptionElement = detail$('.jobDescriptionContent');
            const salaryElement = detail$('.salaryInfo');
            const jobTypeElement = detail$('.jobInfoItem');
            ...
            jobTypeElement.each((_, item) => {
              ...
              if (text.includes('Job Type')) {
                ...
                job.experienceLevel = text.replace('Experience', '').trim();
              }
            });
          } catch (error) {
      ...
      console.error('Glassdoor crawl error:', error);
      return [];
    }
  }
}

export default GlassdoorCrawler;