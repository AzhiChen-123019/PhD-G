...
import { JobCategory, UniversityJobSubType, EnterpriseJobSubType, JobStatus, JobLibraryType } from './job-model';
...
// 模拟API响应数据
const MOCK_API_RESPONSES: Record<string, RawJobData[]> = {
  'LinkedIn': [
    {
      title: '人工智能研究员',
      company: 'Google',
...
// 大学岗位模拟数据
const UNIVERSITY_JOBS: RawJobData[] = [
  {
    title: '人工智能教授',
    company: '斯坦福大学',
    location: '美国',
...
function classifyJobType(job: RawJobData): { category: JobCategory; subType: UniversityJobSubType | EnterpriseJobSubType } {
    ...
    for (const [subType, keywords] of Object.entries(universityKeywords)) {
      if (keywords.some(keyword => title.includes(keyword) || description.includes(keyword))) {
        return {
          ...
          subType: subType as UniversityJobSubType
        };
      }
    }
    ...
    for (const [subType, keywords] of Object.entries(enterpriseKeywords)) {
      if (keywords.some(keyword => title.includes(keyword) || description.includes(keyword))) {
        return {
          ...
          subType: subType as EnterpriseJobSubType
        };
      }
    }
...
// 抓取岗位数据
async function scrapeJobs(config: ScrapeConfig): Promise<RawJobData[]> {
  console.log(`开始从 ${config.platforms.join(', ')} 抓取岗位数据`);
  
  let allJobs: RawJobData[] = [];
...
// 岗位去重
function deduplicateJobs(jobs: RawJobData[]): RawJobData[] {
...
// 保存岗位数据到MongoDB
async function saveJobsToDatabase(jobs: RawJobData[]): Promise<number> {
...
// 导出函数供其他模块使用
export { scrapeJobs, saveJobsToDatabase, classifyJobType };
